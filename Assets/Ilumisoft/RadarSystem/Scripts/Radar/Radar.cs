...
namespace Ilumisoft.RadarSystem
{
    ...
    public class Radar : MonoBehaviour
    {
        ...
        [SerializeField]
        [Tooltip("UI Text to display distance on each locatable icon")]
        private Text distanceTextPrefab;
        ...
        private void OnLocatableAdded(LocatableComponent locatable)
        {
            ...
            if (locatable != null && !locatableIconDictionary.ContainsKey(locatable))
            {
                ...
                // Add distance text UI
                var distanceText = Instantiate(distanceTextPrefab, icon.transform);
            ...
            foreach (var locatable in locatableIconDictionary.Keys)
            {
                ...
                if (locatableIconDictionary.TryGetValue(locatable, out var icon))
                {
                    if (TryGetIconLocation(locatable, out var iconLocation))
                    {
                        ...
                        // Update distance text UI
                        var distanceText = icon.GetComponentInChildren<Text>();
                        if (distanceText != null)
                        {
                            distanceText.text = $"Distance: {GetDistanceToPlayer(locatable).magnitude:F2} m";
